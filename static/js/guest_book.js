	...
	$(document).on("click","#mySubmit, #queryMySubmit", function() {
		...
		if (parseInt(iGuestBookErrorCount) > 0) {
			...
			if(show_check){
				...
				if(!tncode_div){
		            tncode.init('mySubmit',2);
		            tncode_div =true;
		        }else{
		            tncode.show();
		        }
		        $TN.onsuccess(function(){
		        	$("#myform").append('<input type="hidden" name="tn_r" value="'+tncode._mark_offset+'">');
					$("#add_tag").val(1);
					$("#myform").submit();
		        })
		    }
...
$(document).on("click",'#myReSubmit, #queryReSubmit', function() {
	...
	if (parseInt(iGuestBookErrorCount) > 0) {
		...
		if(!tncode_div){
            tncode.init("myReSubmit",2);
            tncode_div =true;
        }else{
            tncode.show();
        }
        $TN.onsuccess(function(){
        	$("#myReForm").append('<input type="hidden" name="tn_r" value="'+tncode._mark_offset+'">');
			$("#re_tag").val(1);
			$("#myReForm").submit();
		})
	}
